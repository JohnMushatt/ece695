...
cond_t CondCreate(lock_t lock) {
  ...
  int intrval;
  cond_t c;
  // Check for bad lock handle
  if (lock < 0 || lock >= MAX_LOCKS || locks[lock].inuse == 0) 
  {
    return INVALID_COND;
  }
  // Disable interrupts
  intrval = DisableIntrs();
  // Find an available condition variable
  for(c=0; c<MAX_CONDS; c++) {
    // If free, set inuse flag
    if(conds[c].inuse==0)
    {
      conds[c].inuse = 1;
      break;
  ...
  // Restore interrupts
  RestoreIntrs(intrval);
  //Checl if we didn't find an available condition variable
  if (c == MAX_CONDS) 
  {
    return INVALID_COND;
  }
  ...
  {
    printf("FATAL ERROR: could not initialize condition waiting queue in CondCreate!\n");
    exitsim();
  }
  // Set lock handle
  conds[c].lock = lock;
...
int CondHandleWait(cond_t c) {
  ...
  Lock *lock;
  Cond *cond;
  // Check for invalid condition variable handle
  if ( c < 0 || c > MAX_CONDS || conds[c].inuse == 0)
  {
    dbprintf('s', "CondHandleWait: Invalid condition variable handle\n");
    return INVALID_COND;
  }
  // Find condition variable and lock
  cond = &conds[c];
  lock = &locks[cond->lock];

  //We know that if we have a valid cond we have a valid lock
  if (lock->pid != GetCurrentPid())
  {
    dbprintf('s', "CondHandleWait: Process %d does not own lock %d\n", GetCurrentPid(), cond->lock);
    return SYNC_FAIL;
  }

  intrval = DisableIntrs();
  // Add to process waiting queue
  link = AQueueAllocLink(( void *)currentPCB);
  if (link == NULL)
  {
    printf("FATAL ERROR: could not allocate link for condition varwaiting queue in CondHandleWait!\n");
    exitsim();
  }
  if (AQueueInsertLast(&cond->waiting, link) != QUEUE_SUCCESS)
  {
    printf("FATAL ERROR: could not insert new link into condition waiting queue in CondHandleWait!\n");
    exitsim();
  }

  //Remove process from lock to adhere to hoare logic
  lock->pid = -1;
  //If there are waiting processes, wake up next in queue
  if( !AQueueEmpty(&lock->waiting))
  {
    Link *llock = AQueueFirst(&lock->waiting);
    PCB *lock_pcb = (PCB *)AQueueObject(llock);
    if (AQueueRemove(&llock) != QUEUE_SUCCESS)
    {
      dbprintf('s', "FATAL ERROR: could not remove link from lock queue in CondHandleWait!\n");
      exitsim();
    }
    //Asign new lock to woken up process
    lock->pid = GetPidFromAddress(lock_pcb);
    ProcessWakeup(lock_pcb);
  }
  ProcessSleep();
  RestoreIntrs(intrval);
  return SYNC_SUCCESS;
...
int CondHandleSignal(cond_t c) {
  ...
  if( c < 0 || c > MAX_CONDS || conds[c].inuse == 0)
  {
    dbprintf('s', "CondHandleSignal: Invalid condition variable handle\n");
    return SYNC_FAIL;
  }
  // Grab actual condition variable and lock
  cond = &conds[c];
  lock = &locks[cond->lock];

  if( lock->pid != GetCurrentPid())
  {
    dbprintf('s', "CondHandleSignal: Process %d does not own lock %d\n", GetCurrentPid(), cond->lock);
    return SYNC_FAIL;
  }
  ...
  if( !AQueueEmpty(&cond->waiting))
  {
    ...
    link = AQueueFirst(&cond->waiting);
    pcb = (PCB *)AQueueObject(link);
    if( AQueueRemove(&link) != QUEUE_SUCCESS)
    {
      dbprintf('s', "FATAL ERROR: could not remove link from condition waiting queue in CondHandleSignal!\n");
      exitsim();
    }
    self_link = AQueueAllocLink((void *)currentPCB);
    if (self_link == NULL)
    {
      dbprintf('s', "FATAL ERROR: could not allocate link for condition waiting queue in CondHandleSignal!\n");
      exitsim();
    }
    if (AQueueInsertLast(&cond->waiting, self_link) != QUEUE_SUCCESS)
    {
      dbprintf('s', "FATAL ERROR: could not insert new link into condition waiting queue in CondHandleSignal!\n");
      exitsim();
    }
    // Set lock pid to pid from waiting process control block
    lock->pid = GetPidFromAddress(pcb);
    // Wakeup but do not give it the lock
    ProcessWakeup(pcb);
    ProcessSleep();
  }
...
int CondHandleBroadcast(cond_t c) {
  ...
  if( c < 0 || c >= MAX_CONDS || conds[c].inuse == 0)
  {
    dbprintf('s', "CondHandleBroadcast: Invalid condition variable handle\n");
    return SYNC_FAIL;
  }
  cond = &conds[c];
  lock = &locks[cond->lock];
  ...
  if( lock->pid != GetCurrentPid())
  {
    dbprintf('s', "CondHandleBroadcast: Process %d does not own lock %d\n", GetCurrentPid(), cond->lock);
    return SYNC_FAIL;
  }
  intrval = DisableIntrs();

  // Hoare-style baton passing to wake all waiters one at a time
  while(!AQueueEmpty(&cond->waiting))
  {
    ...
    if( AQueueRemove(&link) != QUEUE_SUCCESS)
    {
      dbprintf('s', "FATAL ERROR: could not remove link from condition waiting queue in CondHandleBroadcast!\n");
      exitsim();
    }

    // Put current signaler at FRONT of lock queue to regain lock immediately after waiter releases
    self_link = AQueueAllocLink((void *)currentPCB);
    if (self_link == NULL) {
      printf("FATAL ERROR: could not allocate link for lock waiting queue in CondHandleBroadcast!\n");
      exitsim();
    }
    if (AQueueInsertFirst(&lock->waiting, self_link) != QUEUE_SUCCESS) {
      printf("FATAL ERROR: could not insert signaler into lock waiting queue in CondHandleBroadcast!\n");
      exitsim();
    }

    // Transfer lock to waiter and sleep
    lock->pid = GetPidFromAddress(pcb);
    ProcessWakeup(pcb);
    ProcessSleep();
    // When we wake up here, we hold the lock again (via LockRelease priority), and can continue.
  }
