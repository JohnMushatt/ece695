...
void main (int argc, char *argv[])
{
  ...
  if(p_type == 'p')
  {
    ...
    while(index < 10)
    {
      ...
      {
        Printf("PID %d put item %c into buffer\n", pid, str[index]);
        index++;
      }
      else
      {
        Printf("PID %d failed to put item into buffer\n", pid);
        //ProcessSleep();
      }
    }
  }
  else if(p_type == 'c')
  {
    ...
    while( index < 10)
    {
      if(buffer_get(cb, &str[index]) == BUFFER_SUCCESS)
      {
        Printf("PID %d got item %c from buffer\n", pid, str[index]);
        buff[index] = str[index];
        index++;
      }
      else
      {
        Printf("PID %d failed to get item from buffer\n", pid);
      }
    }
  }
